...
async function main(): Promise<void> {
  ...
  const deps: Deps = { config, repo, service, renderer, ask, pending: new Map(), startedAt: new Date() };
  ...
  const runner = run(bot, { runner: { fetch: { allowed_updates: ["message", "callback_query", "inline_query", "my_chat_member"] } } });
