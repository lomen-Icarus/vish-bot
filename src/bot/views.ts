...
export function featuresText(deps: Deps): string {
  ...
  if (deps.students && deps.config.POISK) {
    lines.push("🕵️ «Где студент? Глобал поиск» — внизу списка групп в «👥 Др. группы» и кнопкой в режиме потока: по ФИО находит группу человека и показывает, где он должен быть сейчас по расписанию.");
  }
  // Inline обещаем только когда он действительно включён в BotFather.
  if (deps.inline) lines.push("", `Inline: напиши в любом чате <code>@${deps.botUsername ?? "бот"} 12-23 завтра</code> — вставится расписание. Работает и в группах, подсказка появляется прямо над полем ввода.`);
  const notes: string[] = [];
