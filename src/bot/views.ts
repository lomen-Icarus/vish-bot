...
export function helpText(deps: Deps): string {
  const lines = [
    ...
    "⚙️ <b>Настройки</b> — группа, подгруппа, уведомления об изменениях, напоминания за N часов до первой пары, перед каждой парой, вечером на завтра, тихие часы.",
    "",
    "Бот сам проверяет портал tt.chuvsu.ru каждые несколько минут и присылает изменения только по твоей группе.",
    "",
    "Команды: /today /tomorrow /week /group /settings /changes /help",
  ];
  if (deps.ask) lines.push("💬 /ask &lt;вопрос&gt; — спросить про расписание своими словами.");
  if (deps.config.MEDIA_CHAT_IDS.length) lines.push("📨 /suggest — предложить новость или достижение медиа-ВИШ.");
