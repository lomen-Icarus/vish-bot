...
Лимиты ИИ теперь меняются прямо из бота: «Админка → 🤖 Лимиты ИИ» или `/ailimit user 25`, `/ailimit global 1000`, `/ailimit boost 200` (разово на сегодня), `/ailimit reset`. Значения из `.env` остаются значением по умолчанию, к которому возвращает «♻️ Как в .env».
